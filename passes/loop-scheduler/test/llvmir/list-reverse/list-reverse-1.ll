; ModuleID = 'list-reverse-0-norm.ll'
...
2:                                                ; preds = %10, %1
  %.01 = phi %struct.list_head_s* [ null, %1 ], [ %6, %10 ], !lws_unroll_2 !12, !lws_unrolled_loop !12
  %.0 = phi %struct.list_head_s* [ %0, %1 ], [ %14, %10 ], !ics_resolved_load_use !13
  %3 = icmp ne %struct.list_head_s* %.0, null
...
._crit_edge1:                                     ; preds = %4
  ...
  store %struct.list_head_s* %.01, %struct.list_head_s** %7, align 4, !tbaa !10, !ics_write_early_exit !13
  br label %9
...
10:                                               ; preds = %4
  ...
  %12 = load %struct.list_head_s*, %struct.list_head_s** %11, align 4, !tbaa !10, !ics_resolved_load !13
  %13 = icmp eq %struct.list_head_s** %11, %7, !ics_resolved_load_icmp !13
  %14 = select i1 %13, %struct.list_head_s* %.01, %struct.list_head_s* %12, !ics_resolved_load_select !13
  %15 = getelementptr inbounds %struct.list_head_s, %struct.list_head_s* %6, i32 0, i32 0
  store %struct.list_head_s* %.01, %struct.list_head_s** %7, align 4, !tbaa !10, !ics_moved_write !13
  store %struct.list_head_s* %.0, %struct.list_head_s** %15, align 4, !tbaa !10
  br label %2
}

; Function Attrs: naked noinline nounwind
define dso_local arm_aapcscc void @Reset_Handler() #2 {
  call void asm sideeffect "    ldr    r0, =0xE000ED08\0A    ldr    r1, =g_am_pfnVectors\0A    str    r1, [r0]", ""() #4, !srcloc !14
  call void asm sideeffect "    ldr    sp, [r1]", ""() #4, !srcloc !15
  call void asm sideeffect "    ldr     r0, =_init_data\0A    ldr     r1, =_sdata\0A    ldr     r2, =_edata\0Acopy_loop:\0A        ldr   r3, [r0], #4\0A        str   r3, [r1], #4\0A        cmp     r1, r2\0A        blt     copy_loop\0A", ""() #4, !srcloc !16
  call void asm sideeffect "    ldr     r0, =_sbss\0A    ldr     r1, =_ebss\0A    mov     r2, #0\0Azero_loop:\0A        cmp     r0, r1\0A        it      lt\0A        strlt   r2, [r0], #4\0A        blt     zero_loop", ""() #4, !srcloc !17
  call void asm sideeffect "    bl   main\0A", ""() #4, !srcloc !18
  call void asm sideeffect "    bkpt     ", ""() #4, !srcloc !19
  unreachable
}
...
!10 = !{!4, !5, i64 0}
!11 = !{!4, !9, i64 6}
!12 = !{!"loop_write_scheduler"}
!13 = !{!"ics_scheduler"}
!14 = !{i32 12926, i32 12968, i32 13014}
!15 = !{i32 13092}
!16 = !{i32 13445, i32 13488, i32 13526, i32 13564, i32 13589, i32 13630, i32 13671, i32 13708}
!17 = !{i32 13797, i32 13835, i32 13872, i32 13905, i32 13930, i32 13967, i32 14000, i32 14043}
!18 = !{i32 14154}
!19 = !{i32 14261}
