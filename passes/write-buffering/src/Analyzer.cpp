 ...
 * ---------- Constructors ----------
 */
Analyzer::Analyzer(Function &F, Noelle &noelle) : F{F}, noelle{noelle} {
  /*
   * Debugging
   */
...
void Analyzer::Dump(void) {
  ...
  for (auto Store : AllWrites) {
    ...
    unsigned NumDependences =
        TotalDependencesAfterWriteInParentBasicBlock[Store].size();
...
std::unordered_set<Instruction *> Analyzer::GetReadsAfterWriteInBasicBlock(
    StoreInst *I) {
...
unsigned Analyzer::GetNumberOfReadDependenceConflictsAfterWriteInBasicBlock(
    StoreInst *I) {
...
void Analyzer::_recordWrites(void) {
  ...
  for (auto &B : F)
    for (auto &I : B)
      if (isa<StoreInst>(&I)) AllWrites.insert(cast<StoreInst>(&I));
...
void Analyzer::_recordReadsAfterWriteInParentBasicBlock(void) {
  ...
  for (auto Store : AllWrites) {
    ...
    while (true && (Next) && (Next->getParent() == Parent)) {
      if (false || isa<LoadInst>(Next) || isa<CallInst>(Next))
     ...
     */
    dbg() << *Store
          << " (ro): " << TotalReadsAfterWriteInParentBasicBlock[Store].size()
          << "\n";
...
void Analyzer::_recordDependencesAfterWriteInParentBasicBlock(void) {
  ...
  for (auto Store : AllWrites) {
    ...
    auto Iterator = [Store, &Dependences](Value *DepValue,
     ...
     */
    FDG->iterateOverDependencesFrom(Store, false, /* Control dependences */
