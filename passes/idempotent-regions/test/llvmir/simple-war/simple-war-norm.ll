; ModuleID = '/home/vkortbeek/devel/intermittent-compiler/intermittent-compiler-collection/passes/idempotent-regions/test/arm-code/simple-war/build-noelle/src/simple-war-ics-loop-unroll.ll'
...
define dso_local arm_aapcscc void @foo() #0 {
  %1 = load i32, i32* @global, align 4, !tbaa !3
  %2 = add nsw i32 %1, 5
  store i32 %2, i32* @global, align 4, !tbaa !3
  ret void
}

; Function Attrs: nounwind
define dso_local arm_aapcscc void @bar() #0 {
  %1 = load i32, i32* @other, align 4, !tbaa !3
  %2 = add nsw i32 %1, 1
  store i32 %2, i32* @other, align 4, !tbaa !3
  ret void
}

; Function Attrs: nounwind
define dso_local arm_aapcscc i32 @main() #0 {
  call arm_aapcscc void @foo()
...
3:                                                ; preds = %0
  %4 = load i32, i32* @global, align 4, !tbaa !3
  %5 = add nsw i32 %4, 42
  store i32 %5, i32* @global, align 4, !tbaa !3
  br label %6

6:                                                ; preds = %3, %._crit_edge
  %7 = load i32, i32* @global, align 4, !tbaa !3
  ret i32 %7
}

; Function Attrs: naked noinline nounwind
define dso_local arm_aapcscc void @Reset_Handler() #1 {
  call void asm sideeffect "    ldr    r0, =0xE000ED08\0A    ldr    r1, =g_am_pfnVectors\0A    str    r1, [r0]", ""() #2, !srcloc !7
  call void asm sideeffect "    ldr    sp, [r1]", ""() #2, !srcloc !8
  call void asm sideeffect "    ldr     r0, =_init_data\0A    ldr     r1, =_sdata\0A    ldr     r2, =_edata\0Acopy_loop:\0A        ldr   r3, [r0], #4\0A        str   r3, [r1], #4\0A        cmp     r1, r2\0A        blt     copy_loop\0A", ""() #2, !srcloc !9
  call void asm sideeffect "    ldr     r0, =_sbss\0A    ldr     r1, =_ebss\0A    mov     r2, #0\0Azero_loop:\0A        cmp     r0, r1\0A        it      lt\0A        strlt   r2, [r0], #4\0A        blt     zero_loop", ""() #2, !srcloc !10
  call void asm sideeffect "    bl   main\0A", ""() #2, !srcloc !11
  call void asm sideeffect "    bkpt     ", ""() #2, !srcloc !12
  unreachable
}
...
!6 = !{!"Simple C/C++ TBAA"}
!7 = !{i32 12955, i32 12997, i32 13043}
!8 = !{i32 13121}
!9 = !{i32 13474, i32 13517, i32 13555, i32 13593, i32 13618, i32 13659, i32 13700, i32 13737}
!10 = !{i32 13826, i32 13864, i32 13901, i32 13934, i32 13959, i32 13996, i32 14029, i32 14072}
!11 = !{i32 14183}
!12 = !{i32 14290}
