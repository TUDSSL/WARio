...
# Requires arguments:
#  1 - voltage threshold on
#  2 - voltage threshold off
#  3 - raw trace file (optional, otherwise all)
...
if len(sys.argv) < 3:
    print('usage:')
    print('  parse_traces.py <on_voltage> <off_voltage> [<optional_raw_trace_file>]')

# Set voltage threshold hysteresis over which we assume a microcontroller works
# Threshold unit: [volts]
voltage_threshold_on  = float(sys.argv[1])
voltage_threshold_off = float(sys.argv[2])

print('Using threshold on voltage:', voltage_threshold_on)
print('Using threshold off voltage:', voltage_threshold_off)

# Get the optional raw file argument
try:
    parse_file_arg = sys.argv[3]
    print('Parsing raw file:', parse_file_arg)
except:
    parse_file_arg = None
...
def parse_trace_file(cwd, raw_traces_path, traces_directory, trace_file):
    ...
    # Write statistics
    for x in on_off_difference_samples_list[1:]:
        file.write(str(x) + '\n')
    file.close()
...
for traces_directory in traces_directories:
    ...
    for trace_file in trace_files_list:
        # If we want to parse a specific file
        if (parse_file_arg == None) or (parse_file_arg == trace_file):
            parse_trace_file(cwd, raw_traces_path, traces_directory, trace_file)
